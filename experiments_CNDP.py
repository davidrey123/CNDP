...
def run(net, scenario):
	...
	while True:
	    ...
	    iter, ub, gap, ll_gap, elapsed_time, time_tap, time_cplex, num_oa_cuts = test.solve()
	
	    if iter > 0:
	        with open("log.txt", "a") as f:
	            f.write(str(scenario)+" & "+str(round(ub, 3))+" & "+str(round(gap*100, 2))+"% & "+str(round(ll_gap*100, 2))+"% & "+str(round(elapsed_time, 1))+" & "+str(round(time_cplex, 1))+" & " + str(round(time_tap, 1))+" & "+str(iter)+" & " + str(num_oa_cuts)+"\n")
